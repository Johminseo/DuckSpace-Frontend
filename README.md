...
내 최애 굿즈를 장식장에 전시하고, 팝업스토어 정보를 확인하고, 다른 덕후들과 교환/잡담을 나누는 덕질 공간 서비스입니다.

> 실제 프로젝트 코드는 이 저장소 루트가 아니라 `duckspace/` 서브디렉토리에 있습니다. 아래 명령어는 모두 `duckspace/` 안에서 실행합니다.
...
> `.env` 파일(백엔드 API 주소 등)은 보안상 저장소에 공유되어 있지 않습니다. 로컬 실행을 위해서는 팀 내부적으로 `.env` 파일을 별도로 전달받아야 합니다.
